...
from chronam.core.models import Batch, Title
...
class BatchLoaderTest(TestCase):
    ...
    def test_load_batch(self):
        ...
        self.assertEqual(date.strftime(issue.date_issued, "%Y-%m-%d"), "1911-09-17")
        self.assertEqual(len(issue.pages.all()), 56)

        page = issue.pages.all()[0]
        self.assertEqual(page.sequence, 1)
        self.assertEqual(page.url, "/lccn/sn83045396/1911-09-17/ed-1/seq-1/")
        ...
        self.assertEqual(
            solr_doc["subject"],
            ["Salt Lake City (Utah)--Newspapers.", "Utah--Salt Lake City.--fast--(OCoLC)fst01205314"],
        )
        self.assertEqual(solr_doc["place"], ["Utah--Salt Lake--Salt Lake City"])
        self.assertEqual(
            solr_doc["note"],
            [
        ...
        f = os.path.join(os.path.dirname(chronam.core.__file__), "test-data", "uuml_thys_ocr.txt")
        self.assertEqual(solr_doc["ocr_eng"], open(f).read().decode("utf-8"))
