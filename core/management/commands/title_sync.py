...
class Command(BaseCommand):
    ...
    def find_titles_not_updated(self, limited=True):
        if limited:
            titles = Title.objects.order_by('-version').values('lccn_orig',
                                                           'oclc',
                                                           'version')
    ...
    def handle(self, *args, **options):
        ...
        #TODO: Reassess after we make pull_titles work with states more cleanly.
        if hasattr(settings, "BIB_STORAGE") and os.path.isdir(settings.BIB_STORAGE):
            _logger.info("Starting OCLC pull.")
            #TODO: Add check to make sure that pull_titles destination is empty.
            ...
            for title in tnu:
                ...
                if not essays or not issues:
                    ...
                    _logging.info(error + ' essays ' + error_end)
                    continue
                elif issues:
                    _logging.info(error + ' issues ' + error_end)
                    continue
           
            #TODO: Put in the real holdings directory
            holdings_dir = '/home/jkazil/chronam_files/data/oclcMarcHoldings/oclcMarcHoldings-20120328/data'
