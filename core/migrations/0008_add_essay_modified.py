...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for e in orm.Essay.objects.all():
            # set each modified time to some time far in the past
            # to trigger an update the next time essays are loaded
            e.modified = datetime.datetime(1999, 12, 31, 0, 0)
            e.save()
