...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'OcrDump.name'
        db.delete_column('core_ocrdump', 'name')

        # Deleting field 'OcrDump.sequence'
        db.delete_column('core_ocrdump', 'sequence')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'OcrDump.name'
        raise RuntimeError("Cannot reverse this migration. 'OcrDump.name' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'OcrDump.sequence'
        raise RuntimeError("Cannot reverse this migration. 'OcrDump.sequence' and its values cannot be restored.")
