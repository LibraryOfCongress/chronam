...
def write_sitemaps():
    ...
    for loc, last_mod, newspaper, pub_date, title in sitemap_urls():
        ...
        if newspaper and pub_date and title:
            sitemap.write("<url><loc>http://chroniclingamerica.loc.gov%s</loc><lastmod>%s</lastmod><news:publication><news:name>%s</news:name><news:language>en</news:language></news:publication><news:publication_date>%s</news:publication_date><news:title>%s</news:title></url>\n" % (loc, rfc3339(last_mod), force_escape(newspaper), rfc3339(pub_date), force_escape(title)))
        else:
