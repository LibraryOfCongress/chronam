...
def quality(mime_type, ranges):
    ...
    """ 
    parsed_ranges = [parse_media_range(r) for r in filter_blank(ranges.split(","))]
...
def best_match(supported, header):
    ...
    in order of increasing desirability, in case of situation where
    there is a tie.
    
    >>> best_match(['application/xbel+xml', 'text/xml'], 'text/*;q=0.5,*/*; q=0.1')
    'text/xml'
    """
    parsed_header = [parse_media_range(r) for r in filter_blank(header.split(","))]
    weighted_matches = []
    pos = 0
...
if __name__ == "__main__":
    ...
    class TestMimeParsing(unittest.TestCase):
        ...
        def test_order_best_match(self):
            mime_types_supported = ['text/html', 'application/rdf+xml']
            self.assertEqual(best_match(['text/html', 'application/rdf+xml'], 'text/html, application/rdf+xml'), 'application/rdf+xml')
            self.assertEqual(best_match(['application/rdf+xml', 'text/html'], 'text/html, application/rdf+xml'), 'text/html')
