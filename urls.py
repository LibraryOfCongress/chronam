...
sitemap_index_cached = add_cache_headers(settings.DEFAULT_TTL_SECONDS, settings.SHARED_CACHE_MAXAGE_SECONDS)(
    sitemap_index
)
subsitemap_index_cached = add_cache_headers(
    settings.DEFAULT_TTL_SECONDS, settings.SHARED_CACHE_MAXAGE_SECONDS
)(subsitemap_index)
...
urlpatterns = [
    ...
    url(
        ...
        add_cache_headers(settings.DEFAULT_TTL_SECONDS)(views.home.home),
    ...
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/seq-(?P<sequence>\d+)/coordinates/$",
        add_cache_headers(settings.PAGE_IMAGE_TTL_SECONDS, settings.SHARED_CACHE_MAXAGE_SECONDS)(
            views.image.coordinates
        ),
        name="chronam_page_coordinates",
    ),
    # example: /lccn/sn85066387/1907-03-17/ed-1/seq-4/coordinates/;words=corn+peas+cigars
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/seq-(?P<sequence>\d+)/coordinates/;words=(?P<words>.+)$",
...
urlpatterns += [
    url(r"^about/$", views.about, name="chronam_about"),
    url(r"^help/$", views.help, name="chronam_help"),
    # explainOCR.html
    url(r"^ocr/$", views.ocr, name="chronam_ocr"),
    # API docs
    url(r"^about/api/$", views.about_api, name="chronam_about_api"),
    # example: /lccn/sn85066387
    url(r"^lccn/(?P<lccn>\w+)/$", views.title, name="chronam_title"),
    # example: /lccn/sn85066387/issues/
    url(r"^lccn/(?P<lccn>\w+)/issues/$", views.issues, name="chronam_issues"),
    # example: /lccn/sn85066387/issues/1900
    url(r"^lccn/(?P<lccn>\w+)/issues/(?P<year>\d{4})/$", views.issues, name="chronam_issues_for_year"),
    # example: /lccn/sn85066387/issues/first_pages
    url(
        r"^lccn/(?P<lccn>\w+)/issues/first_pages/$",
        views.issues_first_pages,
        name="chronam_issues_first_pages",
    ),
    # example: /lccn/sn85066387/issues/first_pages/3
    url(
        r"^lccn/(?P<lccn>\w+)/issues/first_pages/(?P<page_number>\d+)/$",
        views.issues_first_pages,
        name="chronam_issues_first_pages_page_number",
    ),
    # example: /lccn/sn85066387/marc
    url(r"^lccn/(?P<lccn>\w+)/marc/$", views.title_marc, name="chronam_title_marc"),
    # example: /lccn/sn85066387/feed/
    url(r"^lccn/(?P<lccn>\w+)/feed/$", views.title_atom, name="chronam_title_atom"),
    ...
    # example: /essays/
    url(r"^essays/$", views.essays, name="chronam_essays"),
    # example: /essays/1/
    url(r"^essays/(?P<essay_id>\d+)/$", views.essay, name="chronam_essay"),
    ...
    url(r"^lccn/(?P<lccn>\w+)/essays/$", views.title_essays, name="chronam_title_essays"),
    ...
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/(?P<page_number>\d+)/$",
        views.issue_pages,
    ...
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/seq-(?P<sequence>\d+)/;words=(?P<words>.+)$",
        views.page_words,
        name="chronam_page_words",
    ),
    # example: /lccn/sn85066387/1907-03-17/ed-1/seq-4/print/image_813x1024_from_0,0_to_6504,8192
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/seq-(?P<sequence>\d+)/print/image_(?P<width>\d+)x(?P<height>\d+)_from_(?P<x1>\d+),(?P<y1>\d+)_to_(?P<x2>\d+),(?P<y2>\d+)/$",
        views.page_print,
        name="chronam_page_print",
    ),
    # example: /lccn/sn85066387/1907-03-17/ed-1/seq-4/ocr/
    url(
        r"^lccn/(?P<lccn>\w+)/(?P<date>\d{4}-\d{2}-\d{2})/ed-(?P<edition>\d+)/seq-(?P<sequence>\d+)/ocr/$",
        views.page_ocr,
        name="chronam_page_ocr",
    ),
    url(r"^newspapers/$", views.newspapers, name="chronam_newspapers"),
    url(
        r"^newspapers/feed/$",
        add_cache_headers(settings.API_TTL_SECONDS)(feeds.NewspaperFeedAtom()),
        name="chronam_newspapers_atom",
    ),
    url(r"^newspapers.(?P<format>csv)$", views.newspapers, name="chronam_newspapers_format"),
    url(r"^newspapers.(?P<format>txt)$", views.newspapers, name="chronam_newspapers_format"),
    url(r"^newspapers.(?P<format>json)$", views.newspapers, name="chronam_newspapers_format"),
    url(r"^newspapers/(?P<state>[^/;]+)/$", views.newspapers, name="chronam_newspapers_state"),
    url(
        r"^newspapers/(?P<state>[^/;]+)\.(?P<format>json)$", views.newspapers, name="chronam_newspapers_json"
    ),
    url("search/pages/opensearch.xml", views.search_pages_opensearch, name="chronam_search_pages_opensearch"),
    url(r"^search/pages/results/$", views.search_pages_results, name="chronam_search_pages_results"),
    url(
        r"^search/pages/results/(?P<view_type>list)/$",
    ...
    url(r"^search/titles/$", views.search_titles, name="chronam_search_titles"),
    url(r"^search/titles/results/$", views.search_titles_results, name="chronam_search_titles_results"),
    url(r"^suggest/titles/$", views.suggest_titles, name="chronam_suggest_titles"),
    url(r"^search/pages/navigation/$", views.search_pages_navigation, name="chronam_search_pages_navigation"),
    url(r"^search/advanced/$", views.search_advanced, name="chronam_search_advanced"),
    url(r"^events/$", views.events, name="chronam_events"),
    url(r"^events\.csv$", views.events_csv, name="chronam_events_csv"),
    ...
    url(r"^titles/;page=(?P<page_number>\d+)$", views.titles, name="chronam_titles_page"),
    # example: /titles;start=F
    url(r"^titles/;start=(?P<start>\w)$", views.titles, name="chronam_titles_start"),
    # example: /titles;start=F;page=5
    url(
        r"^titles/;start=(?P<start>\w);page=(?P<page_number>\d+)$",
        views.titles,
        name="chronam_titles_start_page",
    ),
    # example: /titles/places/pennsylvania
    url(r"^titles/places/(?P<state>[^/;]+)/$", views.titles_in_state, name="chronam_state"),
    # example: /titles/places/pennsylvania;page=1
    url(
        r"^titles/places/(?P<state>[^/;]+)/;page=(?P<page_number>\d+)$",
    ...
    url(
        r"^titles/places/(?P<state>[^/;]+)/(?P<county>[^/;]+)/$",
        views.titles_in_county,
        name="chronam_county",
    ),
    # example /titles/places/pennsylvania/allegheny;page=1
    url(
        r"^titles/places/(?P<state>[^/;]+)/(?P<county>[^/;]+)/;page=(?P<page_number>\d+)$",
        views.titles_in_county,
        name="chronam_county_page_number",
    ),
    # example: /titles/places/pennsylvania/allegheny/pittsburgh
    url(
        r"^titles/places/(?P<state>[^/;]+)/(?P<county>[^/;]+)/(?P<city>[^/;]+)/$",
        views.titles_in_city,
        name="chronam_city",
    ),
    # example: /titles/places/pennsylvania/allegheny/pittsburgh;page=1
    url(
        r"^titles/places/(?P<state>[^/;]+)/(?P<county>[^/]+)/(?P<city>[^/;]+)/;page=(?P<page_number>\d+)$",
        views.titles_in_city,
        name="chronam_city_page_number",
    ),
    # example: # /titles/places/pennsylvania/allegheny/pittsburgh;page=1;order=title
    url(
        r"^titles/places/(?P<state>[^/;]+)/(?P<county>[^/;]+)/(?P<city>[^/;]+)/;page=(?P<page_number>\d+);(?P<order>\w+)$",
        views.titles_in_city,
        name="chronam_city_page_number",
    ),
    # example: /states
    url(r"^states/$", views.states, name="chronam_states"),
    # example: /states_counties/
    url(r"^states_counties/$", views.states_counties, name="chronam_states_counties"),
    # example: /states.json
    url(r"^states\.(?P<format>json)$", views.states, name="chronam_states_json"),
    # example: /counties/pennsylvania
    url(r"^counties/(?P<state>[^/;]+)/$", views.counties_in_state, name="chronam_counties_in_state"),
    # example: /counties/pennsylvania.json
    url(
        r"^counties/(?P<state>[^/;]+)\.(?P<format>json)$",
        views.counties_in_state,
        name="chronam_counties_in_state_json",
    ),
    # example: /cities/pennsylvania/allegheny
    url(
        r"^cities/(?P<state>[^/;]+)/(?P<county>[^/]+)/$",
        views.cities_in_county,
        name="chronam_cities_in_county",
    ),
    # example: /cities/pennsylvania/allegheny.json
    url(
        r"^cities/(?P<state>[^/;]+)/(?P<county>[^/]+)\.(?P<format>json)$",
        views.cities_in_county,
        name="chronam_cities_in_county_json",
    ),
    # example: /cities/pennsylvania
    url(r"^cities/(?P<state>[^/;]+)/$", views.cities_in_state, name="chronam_cities_in_state"),
    # example: /cities/pennsylvania.json
    url(
        r"^cities/(?P<state>[^/;]+)\.(?P<format>json)$",
        views.cities_in_state,
        name="chronam_cities_in_state_json",
    ),
    # example: /institutions
    url(r"^institutions/$", views.institutions, name="chronam_institutions"),
    # example: /institutions;page=5
    url(
        r"^institutions/;page=(?P<page_number>\d+)$",
        views.institutions,
        name="chronam_institutions_page_number",
    ),
    # example: /institutions/cuy
    url(r"^institutions/(?P<code>[^/]+)/$", views.institution, name="chronam_institution"),
    # example: /institutions/cuy/titles
    url(
        r"^institutions/(?P<code>[^/]+)/titles/$", views.institution_titles, name="chronam_institution_titles"
    ),
    # example: /institutions/cuy/titles/5/
    url(
        r"^institutions/(?P<code>[^/]+)/titles/(?P<page_number>\d+)/$",
...
urlpatterns += [
    ...
    # flickr report
    url(r"^flickr/$", views.pages_on_flickr, name="chronam_pages_on_flickr"),
    # batch summary
    url(r"^batches/summary/$", views.batch_summary, name="chronam_batch_summary"),
    url(r"^batches/summary.(?P<format>txt)$", views.batch_summary, name="chronam_batch_summary_txt"),
    # batch view
    url(r"^batches/$", views.batches, name="chronam_batches"),
    url(r"^batches/;page=(?P<page_number>\d+)$", views.batches, name="chronam_batches_page"),
    url(r"^batches/feed/$", views.batches_atom, name="chronam_batches_atom"),
    ...
    url(r"^reel/(?P<reel_number>\w+)/$", views.reel, name="chronam_reel"),
    # languages
    url(r"^languages/$", views.languages, name="chronam_languages"),
    url(r"^languages/(?P<language>.+)/batches/$", views.language_batches, name="chronam_language_batches"),
    url(
        r"^languages/(?P<language>.+)/batches/;page=(?P<page_number>\d+)$",
        views.language_batches,
        name="chronam_language_batches_page_number",
    ),
    url(r"^languages/(?P<language>.+)/titles/$", views.language_titles, name="chronam_language_titles"),
    url(
        r"^languages/(?P<language>.+)/titles/;page=(?P<page_number>\d+)$",
        views.language_titles,
        name="chronam_language_titles_page_number",
    ),
    url(
        r"^languages/(?P<language>.+)/(?P<batch>.+)/(?P<title>.+)/$",
        views.language_pages,
        name="chronam_language_title_pages",
    ),
    url(
        r"^languages/(?P<language>.+)/(?P<batch>.+)/(?P<title>.+)/;page=(?P<page_number>\d+)$",
        views.language_pages,
        name="chronam_language_title_pages_page_number",
    ),
    url(
        r"^languages/(?P<language>.+)/(?P<batch>.+)/$",
        views.language_pages,
        name="chronam_language_batch_pages",
    ),
    url(
        r"^languages/(?P<language>.+)/(?P<batch>.+)/;page=(?P<page_number>\d+)$",
        views.language_pages,
...
urlpatterns += [url(r"^data/(?P<path>.*)$", serve, {"document_root": _MEDIA_ROOT}, name="chronam_data_files")]
