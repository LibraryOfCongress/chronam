...
jQuery(function($) {
  ...
  $("#tabs-container").on("chronam.tabsloaded", function() {
    ...
    $("#id_date_from").datepicker({
      defaultDate: new Date(selected_start_year, 1 - 1, 1),
      showOn: "focus",
    ...
    $("#id_date_to").datepicker({
      defaultDate: new Date(selected_end_year, 12 - 1, 31),
      showOn: "focus",
      yearRange: min_year + ":" + max_year,
      currentText: "",
      changeMonth: true,
    ...
    if (vars["dateFilterType"] == "range") {
      $("#id_radiorange").attr("checked", "checked");
      $("select#id_date2 option[value='" + selected_end_year + "']").attr(
        "selected",
        true
      );
    ...
    $("form#fulltext2").submit(function(event) {
      event.preventDefault();
      if (
        $("#fulltext2 input[type=radio]:checked").attr("id") == "id_radioyear"
      ) {
