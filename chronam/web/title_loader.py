    ...
    def load_bib(self, record):
        ...
        try:
            ...
            if title.version==dt:
                ...
                # XXX: this will only work when building up a chronam db 
                # from nothing. If we load an updated title record for a 
                # title that we have issue data for this call to delete()
                # will also delete issues and pages that are hanging off of it!
                title.delete()
                # create new one from this newer record
